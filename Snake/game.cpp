...
Game::Game()
{
    this -> resize(DOT_WIDTH * FIELD_WITH, DOT_HEIGHT * FIELD_HEIGHT);
    this -> setWindowTitle("Tuti snake");
    initGame();
}
...
void Game::doDrawing()
{
    ...
    if (m_inGame) {
        qp.setBrush(Qt::red);
        qp.drawEllipse(m_apple.x() * DOT_WIDTH, m_apple.ry() * DOT_HEIGHT, DOT_WIDTH, DOT_HEIGHT);
...
{
//    Qtime time = Qtime::curentTime();
    m_apple.rx() =  QRandomGenerator::global()->generate() % DOT_WIDTH; // почему-т qrand не сработал
    m_apple.ry() =  QRandomGenerator::global()->generate() % DOT_HEIGHT;
}
...
void Game::check_field()
{
   ...
   if (m_dots[0].x() >= FIELD_WITH) m_inGame = false;
